  ...
  return (
    <div className="absolute inset-0 pointer-events-none z-10">
      ...
      <div className="absolute top-3 right-3 sm:top-4 sm:right-4 text-right">
        <p className="text-xl sm:text-3xl font-bold text-white font-mono tabular-nums">
      ...
      {/* Combo */}
      {combo > 1 && (
        <div className="absolute top-1/4 sm:top-1/3 left-1/2 -translate-x-1/2 text-center">
          <p
            className="text-3xl sm:text-5xl font-black text-white/90 tabular-nums"
            style={{
              textShadow:
                combo >= 50
          ...
          <p className="text-[10px] sm:text-sm font-bold text-white/60 tracking-widest uppercase">
            combo
          </p>
        </div>
      )}
          ...
          {showVolumeSlider && (
            <div className="flex items-center gap-2 bg-black/60 backdrop-blur-sm rounded-full px-3 py-1.5 border border-white/10">
              <input
                ...
                className="w-20 sm:w-24 h-1 accent-[#4ecdc4] cursor-pointer"
                aria-label="Volume"
              />
